-- =====================================================================
-- 035_wipes_in_stock.sql
-- Ensure Pure & Gentle Water Baby Wipes are purchasable (active + stock).
-- Safe to re-run.
-- =====================================================================
...
-- Seed sellable inventory for active wipes variants (split product stock evenly)
insert into public.inventory (product_variant_id, warehouse_id, quantity, reserved)
select
  pv.id,
  wh.id,
  greatest(floor(coalesce(p.stock, 0)::numeric / 2)::integer, 250),
  0
from public.products p
join public.product_variants pv on pv.product_id = p.id and pv.is_active = true
cross join lateral (
  select id from public.warehouses where is_active = true order by is_default desc nulls last, created_at asc limit 1
) wh
where p.slug = 'pure-gentle-water-baby-wipes'
  and p.deleted_at is null
on conflict (product_variant_id, warehouse_id) do update
set
  quantity = greatest(excluded.quantity, public.inventory.quantity),
  updated_at = now();
