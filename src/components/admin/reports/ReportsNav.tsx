...
import { REPORT_NAV } from "@/lib/admin/report-types";
...
export default function ReportsNav() {
  ...
  return (
    <nav className="flex gap-1 overflow-x-auto border-b border-cream-200 pb-1" aria-label="Reports navigation">
      {REPORT_NAV.map((item) => {
        const active = pathname === item.href || (item.href !== "/admin/reports" && pathname.startsWith(item.href));
        return (
          <Link
            key={item.href}
            href={item.href}
