...
function toCategoryRow(input: CategoryInput): TablesInsert<"categories"> {
  return {
    name: input.name,
    slug: input.slug,
...
export async function createCategoryAction(
  ...
  if (error || !data) {
    if (error?.code === "23505") {
      return { ok: false, error: "A category with this slug already exists.", fieldErrors: { slug: "Already in use" } };
    }
...
export async function updateCategoryAction(
  ...
  if (parsed.data.parent_id === id) {
    return { ok: false, error: "A category cannot be its own parent.", fieldErrors: { parent_id: "Invalid parent" } };
  }

  const supabase = await createSupabaseServerClient();
  const { error } = await supabase
    .from("categories")
    .update(toCategoryRow(parsed.data))
    .eq("id", id);
  ...
  return { ok: true, error: null, categoryId: id };
}
...
export async function softDeleteCategory(id: string) {
  ...
  await supabase
    ...
    .eq("id", id);
  await supabase.rpc("log_audit", { p_table: "categories", p_record: id, p_action: "delete" });
  revalidatePath("/admin/categories");
}
...
export async function restoreCategory(id: string) {
  ...
  await supabase.rpc("log_audit", { p_table: "categories", p_record: id, p_action: "update", p_new: { restored: true } });
  revalidatePath("/admin/categories");
}
...
export async function duplicateCategory(id: string) {
  ...
  const { data: copy } = await supabase
    .from("categories")
    .insert({
      ...(rest as TablesInsert<"categories">),
  ...
  if (copy) {
    await supabase.rpc("log_audit", { p_table: "categories", p_record: copy.id, p_action: "insert", p_new: { duplicated_from: id } });
    revalidatePath("/admin/categories");
...
export async function bulkUpdateCategoryStatus(ids: string[], status: CatalogStatus) {
  await ensureCatalogAccess();
...
export async function bulkSoftDeleteCategories(ids: string[]) {
  await ensureCatalogAccess();
  if (!ids.length) return;
  const supabase = await createSupabaseServerClient();
  await supabase
