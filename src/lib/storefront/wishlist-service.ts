...
export async function listWishlistProductIds(): Promise<string[]> {
...
export async function toggleWishlistProduct(productId: string): Promise<WishlistActionResult> {
  if (!productId?.trim()) {
...
export async function removeWishlistProduct(productId: string): Promise<WishlistActionResult> {
  const customerId = await resolveCustomerId();
